(function(i){var g={},c,l,h,a,e=750,m;function j(p,o,r,q){return Math.abs(p-o)>=Math.abs(r-q)?(p-o>0?"Left":"Right"):(r-q>0?"Up":"Down");}function n(){a=null;
if(g.last){g.el.trigger("longTap");g={};}}function d(){if(a){clearTimeout(a);}a=null;}function f(){if(c){clearTimeout(c);}if(l){clearTimeout(l);}if(h){clearTimeout(h);
}if(a){clearTimeout(a);}c=l=h=a=null;g={};}function k(o){return(o.pointerType=="touch"||o.pointerType==o.MSPOINTER_TYPE_TOUCH)&&o.isPrimary;}function b(p,o){return(p.type=="pointer"+o||p.type.toLowerCase()=="mspointer"+o);
}i(document).ready(function(){var q,t,p=0,o=0,s,r;if("MSGesture" in window){m=new MSGesture();m.target=document.body;}i(document).bind("MSGestureEnd",function(v){var u=v.velocityX>1?"Right":v.velocityX<-1?"Left":v.velocityY>1?"Down":v.velocityY<-1?"Up":null;
